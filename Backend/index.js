...
mongoose.connect("mongodb+srv://nickuk32:YkFrIaj6X6qs8L51@cluster0.ngkzzk7.mongodb.net/csProject")
...
app.post("/signup", async (req,res)=>{
    ...
    const token = jwt.sign(data, "secret_project");
...
//Creating Middleware to fetch user
    const fetchUser = async (req,res,next) => {
        ...
        if (!token) {
            res.status(401).send({errors:"Please authenticate usinng valid token"});
        }
        else{
            try {
                const data = jwt.verify(token,"secret_project");
                req.user = data.user;
                next();
            } catch (error) {
